...
class LocalSearchEnv(BaseEnv):
    ...
    def __init__(self, max_turns: int = 2, extra_info: dict = None):
        ...
        self.search_url = extra_info.get("search_url", "http://127.0.0.1:8000/retrieve")
        self.topk = extra_info.get("topk", 3)
        self.timeout = extra_info.get("timeout", DEFAULT_TIMEOUT)
        self.log_requests = extra_info.get("log_requests", True)
    ...
    def _search(self, query: str) -> str:
        ...
        result_text = "<interpreter>Search request failed or timed out after retries.</interpreter>"

        if error_msg:
            metadata["status"] = "api_error"
            result_text = f"<interpreter>Search error: {error_msg}</interpreter>"
            ...
            try:
                ...
                if raw_results:
                    ...
                    final_result = "\n---\n".join(pretty_results)
                    result_text = f"<interpreter>{final_result}</interpreter>"
                    ...
                    result_text = "<interpreter>No search results found.</interpreter>"
                ...
                result_text = f"<interpreter>{error_msg}</interpreter>"
                metadata["status"] = "processing_error"
                logger.error(f"Batch search: {error_msg}")
        else:
            metadata["status"] = "unknown_api_state"
            result_text = "<interpreter>Unknown API state (no response and no error message).</interpreter>"
            logger.error("Batch search: Unknown API state.")

        return result_text

    @with_timeout_and_retry(timeout_seconds=5.0, max_attempts=3)
    async def exec_tool_call(self, action: str) -> tuple[str, float, bool, dict]:
        """
        Args:
            action: str, the search query from the agent.
        Returns:
        ...
        assert "<answer>" not in action, "The <answer> action should be handled in the agent."
        search_result = self._search(action)
