...
from verl_patch.workers.config.model import HFModelConfig
from verl_patch.workers.config.rollout import RolloutConfig
...
class RolloutWorker(Worker):
    def __init__(self, config: RolloutConfig, model_config: HFModelConfig) -> None:
        ...
        if self.config.name == "vllm":
            from verl_patch.workers.code.rollout.vllm_rollout.vllm_rollout_spmd import (
                vLLMRollout,
            )
