...
Its core capabilities include computing importance sampling weights,
filtering outlier samples via rejection sampling, and
...
1. **Multi-Granularity Aggregation**:
   - Importance Sampling (IS):
        Token-level
        Turn-level
        Sequence-level (cross-turn aggregation)
   - Rejection Sampling (RS):
        Token/turn/turn_geo (turn-level geometric mean)
...
- compute_rollout_importance_weights_and_rejection_mask() : compute IS weights + rejection mask + veto (most common for LLM training).
- compute_rollout_importance_weights(): only compute truncated IS weights (for variance reduction, no outlier rejection).
- compute_rollout_rejection_mask(): only filter outliers (for sample cleaning, no IS weight computation).
...
def compute_rollout_rejection_mask(
    ...
    Args:
        ...
        response_mask: Binary mask for valid tokens (1=valid, 0=padding),
            shape (batch_size, seq_length).
        max_turns: Maximum number of conversation turns (used for sequence-level aggregation),
            default 1.
        rollout_rs: Rejection sampling aggregation level, must be one of:
            - "token": Per-token outlier detection
            - "turn": Aggregate per conversation turn
            - "turn_geo": Geometric mean per conversation turn
            - "sequence": Aggregate across entire sequence (requires max_turns > 1)
            - "geometric": Geometric mean across entire sequence (requires max_turns > 1)
    ...
    Returns:
        Tuple containing:
            ...
            metrics: Dictionary of rejection sampling metrics (all scalars), including:
                ...
                - rollout_is_masked_fraction: Overall rejection rate
                - rollout_is_seq_masked_fraction: Sequence-level rejection rate
    """
    # Validate input parameters
    valid_rs_levels = {"token", "turn", "turn_geo", "sequence", "geometric"}
    if rollout_rs not in valid_rs_levels:
        raise ValueError(f"Invalid rollout_rs: {rollout_rs}. Must be one of {valid_rs_levels}.")
    if max_turns == 1 and rollout_rs in ['turn', 'turn_geo']:
        raise ValueError(f"single turn can not use rollout_rs: {rollout_rs}.")
    ...
    if rollout_rs == "token":
    ...
    elif (rollout_rs == "turn" and max_turns > 1) or (rollout_rs == "sequence" and max_turns == 1):
        # Turn-level (multi-turn)/Sequence-level (single-turn) IS weight: product of token ratios (exp(sum(log ratios)))
    ...
    elif (rollout_rs == "turn_geo" and max_turns > 1) or (rollout_rs == "geometric" and max_turns == 1):
        # Turn-level (multi-turn)/Sequence-level (single-turn) geometric mean: exp(mean(log ratios))
    ...
    elif rollout_rs == "sequence" and max_turns > 1:
        # Sequence-level (multi-turn) IS weight: product of all token ratios across turns
        shaped_log_ratio: torch.Tensor = log_ratio.reshape(-1, max_turns, log_ratio.size(-1)).reshape(
            -1, max_turns * log_ratio.size(-1)
        )
        shaped_response_mask: torch.Tensor = response_mask.reshape(-1, max_turns, response_mask.size(-1)).reshape(
            -1, max_turns * response_mask.size(-1)
        )

        log_ratio_sum: torch.Tensor = verl_F.masked_sum(shaped_log_ratio, shaped_response_mask, axis=-1)
        # Reshape back to match original batch dimensions
        log_ratio_sum = log_ratio_sum.unsqueeze(-1).tile([1, max_turns]).reshape(-1).unsqueeze(-1)
        log_ratio_for_metrics = log_ratio_sum

        log_ratio_sum_safe: torch.Tensor = torch.clamp(log_ratio_sum, min=-SAFETY_BOUND, max=SAFETY_BOUND)
        rollout_is_weights = torch.exp(log_ratio_sum_safe).expand_as(log_ratio)

    elif rollout_rs == "geometric" and max_turns > 1:
        # Sequence-level (multi-turn) geometric mean: exp(mean(log ratios) across turns)
        shaped_log_ratio: torch.Tensor = log_ratio.reshape(-1, max_turns, log_ratio.size(-1)).reshape(
            -1, max_turns * log_ratio.size(-1)
        )
        shaped_response_mask: torch.Tensor = response_mask.reshape(-1, max_turns, response_mask.size(-1)).reshape(
        ...
        log_ratio_mean: torch.Tensor = verl_F.masked_mean(shaped_log_ratio, shaped_response_mask, axis=-1)
        # Reshape back to match original batch dimensions
        log_ratio_mean = log_ratio_mean.unsqueeze(-1).tile([1, max_turns]).reshape(-1).unsqueeze(-1)
        log_ratio_for_metrics = log_ratio_mean
...
def compute_rollout_importance_weights(
    ...
    Args:
        ...
        rollout_is: IS weight aggregation level, must be one of:
            ...
            - "turn": Per-turn weight (product of tokens; unbiased, high variance)
            - "sequence": Per-sequence weight (product across turns; requires max_turns > 1)
    ...
    """
    # Validate input parameters
    valid_is_levels = {"token", "turn", "sequence"}
    if rollout_is not in valid_is_levels:
        raise ValueError(f"Invalid rollout_is: {rollout_is}. Must be one of {valid_is_levels}.")
    if max_turns == 1 and rollout_is == "turn":
        raise ValueError(f"single turn can not use rollout_is: {rollout_is}.")
    ...
    elif (rollout_is == "turn" and max_turns > 1) or (rollout_is == "sequence" and max_turns == 1):
        # Turn-level (multi-turn)/Sequence-level (single-turn) IS weight: product of token ratios (exp(sum(log ratios)))
        log_ratio_sum: torch.Tensor = verl_F.masked_sum(log_ratio, response_mask, axis=-1).unsqueeze(
            -1
        )  # Shape: (batch_size, 1)
    ...
    elif rollout_is == "sequence" and max_turns > 1:
        # Sequence-level (multi-turn) IS weight: product of all token ratios across turns
        shaped_log_ratio: torch.Tensor = log_ratio.reshape(-1, max_turns, log_ratio.size(-1)).reshape(
            -1, max_turns * log_ratio.size(-1)
        )
...
def compute_is_metrics(
    ...
    This function calculates statistics for IS weights, using log-space
    ...
    Args:
        ...
        response_mask: Binary mask for valid tokens (1=valid, 0=padding),
            shape (batch_size, seq_length).
        rollout_is: IS weight aggregation level (matches compute_rollout_importance_weights).
    ...
    # Compute metrics based on aggregation level
    if rollout_is in ["sequence", "turn"]:
        # Sequence/turn-level aggregation: use log-space for unclamped stats
        log_max: torch.Tensor = log_ratio_for_metrics.max()
        log_min: torch.Tensor = log_ratio_for_metrics.min()
...
def compute_rollout_importance_weights_and_rejection_mask(
    ...
    Args:
        ...
        max_turns: Maximum number of conversation turns (for sequence-level aggregation),
            default 1.
        rollout_is: IS weight aggregation level (see compute_rollout_importance_weights for options).
    ...
    Returns:
        Tuple containing:
            rollout_is_weights: Truncated IS weights (None if rollout_is is None),
                shape (batch_size, seq_length).
    ...
    if rollout_is is not None and rollout_is_threshold is not None:
        rollout_is_weights, is_metrics = compute_rollout_importance_weights(
            log_ratio=log_ratio,
            response_mask=response_mask,
            max_turns=max_turns,
            rollout_is=rollout_is,
...
def compute_mismatch_metrics(
    ...
    if rollout_log_prob is not None:
        ...
        metrics["mismatch_chi2_token"] = chi2_token.detach().item()
        ...
        metrics["mismatch_chi2_seq"] = chi2_seq.detach().item()
