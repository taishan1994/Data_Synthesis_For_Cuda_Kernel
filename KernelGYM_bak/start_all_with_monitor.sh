...
# Default feedback environment to the first 4 GPUs unless explicitly overridden.
GPU_DEVICES="${FEEDBACK_GPU_DEVICES:-[0,1]}"
...
REDIS_HOST="${REDIS_HOST:-192.168.16.4}"
# REDIS_PORT="${REDIS_PORT:-6379}"
# REDIS_HOST=192.168.16.17s
REDIS_PORT=6379
...
nohup env API_WORKERS=1 python -m kernelgym.server.api.server > "${ROOT_DIR}/${LOG_DIR}/api_server.log" 2>&1 &
API_PID=$!
...
for gpu in ${GPU_LIST}; do
    ...
    nohup python -m kernelgym.worker.single_worker \
        --worker-id "${WORKER_ID}" \
        --device "cuda:${gpu}" \
        --persistent \
...
nohup python -m kernelgym.worker.worker_monitor --persistent > "${ROOT_DIR}/${LOG_DIR}/worker_monitor.log" 2>&1 &
MONITOR_PID=$!
