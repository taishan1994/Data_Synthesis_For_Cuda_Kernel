...
class _HybridHttpWorker:
    ...
    def _extract_best_error_message(self, payload: Dict[str, Any]) -> str:
        candidates = [
        ...
        for value in candidates:
            if value:
                ...
                if text and text.lower() != "task failed":
                    return text

        status = payload.get("status")
        return f"Task {status}" if status else "Task failed"
    ...
    def _build_terminal_response(
        ...
        *,
        ...
        if fetch_error:
            response["result_fetch_error"] = fetch_error

        response["error_message"] = self._extract_best_error_message(response)
        return response
    ...
    def submit_and_poll(self, task_data: Dict[str, Any], client_timeout: int, max_retries: Optional[int]) -> Dict[str, Any]:
        ...
        request_timeout = httpx.Timeout(
            connect=10.0,
            read=max(float(client_timeout), float(self.default_timeout)),
            write=10.0,
            pool=5.0,
        )
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if status in ("completed", "failed", "timeout", "cancelled"):
                            payload.setdefault("task_id", task_data.get("task_id", ""))
                            return payload
                        break
...
class KernelRewardClient:
    ...
    def _extract_error_message(self, result: Dict[str, Any]) -> str:
        ...
        status_payload = result.get("status_payload")
    ...
    def _build_failed_reward_result(self, result: Dict[str, Any], penalty_score: float) -> Dict[str, Any]:
        error_message = self._extract_error_message(result)
        print(f"[HybridClient] reward error_message: {error_message}")
        print(f"[HybridClient] failed raw result: {result}")
    ...
    def calculate_reward_like_kernel(self, result: Dict[str, Any]) -> Dict[str, Any]:
        if result.get("status") != "completed":
            return self._build_failed_reward_result(result, -1.0)
    ...
    def calculate_reward_weighted(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if result.get("status") != "completed":
            return self._build_failed_reward_result(result, penalty_score)
