...
KERNEL_SECTION_MARKERS = ("### CUDA_KERNELS", "### APPLY_BINDINGS", "### MODEL_NEW")
SFT_FEEDBACK_FIELD_ORDER = (
    "task_id",
    "status",
    "compiled",
    "correctness",
...
def _truncate_kernel_response_after_first_complete_block(response: str) -> str:
    """Clip repeated self-answers after the first complete kernel block.

    Kernel generations should end after the first full
    CUDA_KERNELS/APPLY_BINDINGS/MODEL_NEW block. When the model ignores EOS or
    keeps sampling, it may start a second think/answer cycle in the same
    completion. We keep the first complete block and drop the trailing cycle.
    """
    if not response:
        return response

    first_marker = KERNEL_SECTION_MARKERS[0]
    second_marker = KERNEL_SECTION_MARKERS[1]
    third_marker = KERNEL_SECTION_MARKERS[2]

    first_pos = response.find(first_marker)
    if first_pos < 0:
        return response

    second_pos = response.find(second_marker, first_pos + len(first_marker))
    if second_pos < 0:
        return response

    third_pos = response.find(third_marker, second_pos + len(second_marker))
    if third_pos < 0:
        return response

    cut_candidates = []
    next_kernel_cycle = response.find(f"\n{first_marker}", third_pos + len(third_marker))
    if next_kernel_cycle >= 0:
        cut_candidates.append(next_kernel_cycle)

    next_think = response.find("\n<think>", third_pos + len(third_marker))
    if next_think >= 0:
        cut_candidates.append(next_think)

    if not cut_candidates:
        return response

    return response[: min(cut_candidates)].rstrip()


def _canonicalize_feedback_state_for_sft(env_state: dict[str, Any] | None) -> dict[str, Any]:
    """Match the SFT feedback JSON schema and top-level field order exactly."""
    env_state = dict(env_state or {})
    canonical = {}
    for key in SFT_FEEDBACK_FIELD_ORDER:
        canonical[key] = env_state.get(key, None)
    ...
    if error_text:
        ...
        # Keep a compact, single-line summary visible even when Ray log
        # deduplication folds long traceback lines in the console.
        summary = error_text.splitlines()[-1].strip() if error_text.splitlines() else error_text
        metadata.setdefault("error_summary", summary[:1000])
        if canonical.get("error_message") is None:
            canonical["error_message"] = error_text
    ...
    async def _async_rollout_a_prompt(
        ...
        reward_kwargs = {
            **kwargs,
            ...
            "generation_finish_reason": results.outputs[0].finish_reason,
            "rollout_finish_reason": "single_turn",
        }
...
class MultiTurnAsyncvLLMEngine:
    ...
    async def _process_single_turn(
        ...
        params["min_tokens"] = max(int(params.get("min_tokens", 0) or 0), 2)
        ...
        if self.agent_type == "KernelAgent":
            truncated_response = _truncate_kernel_response_after_first_complete_block(response)
            if truncated_response != response:
                truncated_ids = self.tokenizer(
                    truncated_response,
                    add_special_tokens=False,
                )["input_ids"]
                if truncated_ids and len(truncated_ids) <= len(response_token_ids):
                    logging.info(
                        "Trimmed trailing kernel continuation | request_id=%s | orig_tokens=%s | trimmed_tokens=%s",
                        request_id,
                        len(response_token_ids),
                        len(truncated_ids),
                    )
                    response = truncated_response
                    response_token_ids = response_token_ids[: len(truncated_ids)]

        if self.agent_type == "KernelAgent" and self._should_retry_kernel_generation(response_token_ids, response):
            retry_params = dict(params)
            if max_tokens >= 512:
                retry_params["min_tokens"] = max(int(retry_params.get("min_tokens", 0) or 0), 512)
            else:
                retry_params["min_tokens"] = min(int(retry_params.get("min_tokens", 0) or 0), max_tokens)
                logging.info(
                    "Skipping long-min-token kernel retry because remaining window is small | request_id=%s | max_tokens=%s",
                    request_id,
                    max_tokens,
                )
            retry_params["min_tokens"] = min(int(retry_params.get("min_tokens", 0) or 0), max_tokens)
            ...
            retry_results = None
            if retry_params["min_tokens"] <= max_tokens:
                retry_llm_params = SamplingParams(max_tokens=max_tokens, **retry_params)
                retry_request_id = f"{request_id}_retry"
                try:
                    retry_results = await self._run_generation(
                        prompt=prompt,
                        llm_params=retry_llm_params,
            ...
            if retry_results is not None and retry_results.outputs:
                retry_output = retry_results.outputs[0]
                retry_response = retry_output.text
                retry_token_ids = retry_output.token_ids
                if not self._should_retry_kernel_generation(retry_token_ids, retry_response):
                    logging.info(
                        "Kernel retry recovered malformed response | request_id=%s | orig_tokens=%s | retry_tokens=%s",
                        request_id,
                        len(response_token_ids),
                        len(retry_token_ids),
                    )
                    ...
                    response_token_ids = retry_token_ids
        ...
        reward_kwargs = {
            ...
            "prompt_messages": messages,
            "generation_finish_reason": output.finish_reason,
            "rollout_finish_reason": "agent_env_turn",
        }
        ...
        env_state = _canonicalize_feedback_state_for_sft(env_result["env_state"])
    ...
    def _should_retry_kernel_generation(self, response_token_ids: list[int], response_text: str) -> bool:
        if not response_token_ids:
            return True
        if len(response_token_ids) == 1 and response_text.strip() == "":
            return True
        if any(marker in response_text for marker in KERNEL_SECTION_MARKERS):
            return False
        normalized = response_text.strip()
        if not normalized:
            return True
        if normalized.startswith("<think>") and "</think>" not in normalized:
            return True
        return False

    async def _run_generation(self, prompt, llm_params, request_id: str, async_timeout: float | None):
        ...
        async with vllm_timeout(async_timeout):
            async for res in self.engine.generate(prompt=prompt, sampling_params=llm_params, request_id=request_id):
                results = res
        return results
