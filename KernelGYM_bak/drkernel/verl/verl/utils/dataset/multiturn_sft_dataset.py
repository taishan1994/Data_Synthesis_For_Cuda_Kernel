...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        def series_to_item(ls):
            ...
            # Handle single-element Series or ndarray
            while isinstance(ls, pandas.core.series.Series | numpy.ndarray) and len(ls) == 1:
                # If unwrapping would result in a dictionary (a single message),
                # stop unwrapping so it remains a collection of messages.
                if isinstance(ls[0] if isinstance(ls, numpy.ndarray) else ls.iloc[0], dict):
                    break
                ls = ls[0] if isinstance(ls, numpy.ndarray) else ls.iloc[0]
            
            # Handle multi-element ndarray - convert to list
    ...
    def _process_message_tokens(
        ...
        if is_assistant:
            ...
            # The trainer shifts labels by one position before applying loss_mask.
            # Mark the last generation-prompt token so the first assistant token is supervised.
            generation_prompt_prefix = max(len(generation_prompt_tokens) - 1, 0)
            loss_mask = [0] * generation_prompt_prefix + [1] * (len(message_tokens) - generation_prompt_prefix)
        else:
