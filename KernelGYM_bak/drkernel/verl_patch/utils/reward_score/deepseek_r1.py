...
def qwen_math_extract_equal_with_timeout_ray(
    sequence, reference, include_percentage=True, is_close=True, timeout_duration=3
):
    """
    ...
    try:
        # 启动远程任务并等待结果
        future = _remote_qwen_math_equal.remote(
            sequence=sequence, reference=reference, include_percentage=include_percentage, is_close=is_close
        )
...
def compute_score(solution_str, ground_truth, method='strict'):
    ...
    if format_matched:
        # 如果格式正确，则只用 <answer> 内容来比对答案
        model_answer = answer_match.group(1).strip()
        answer_matched = qwen_math_extract_equal_with_timeout_ray(sequence=model_answer, reference=ground_truth)
    else:
        # 如果格式不正确，则换一种方式检测答案
        # （示例：直接判断整段内容里是否能匹配出正确答案；
        #   也可以根据业务需要使用其他逻辑）
        answer_matched = qwen_math_extract_equal_with_timeout_ray(
            sequence=sequence_after_assistant, reference=ground_truth
        )
    # 5) 最终根据四种情况打分
